...
export function CartDrawer() {
  ...
  const updateQty = async (itemId: string, quantity: number) => {
    ...
    try {
      ...
      toast.error(t('components.cartDrawer.updateFailed'));
    } finally {
      setBusy(false);
    }
  };
  ...
  const removeItem = async (itemId: string) => {
    ...
    try {
      ...
      toast.error(t('components.cartDrawer.removeFailed'));
    } finally {
      setBusy(false);
    }
  };
      ...
      <SheetContent side="right" className="flex w-full flex-col border-l border-border bg-background p-0 sm:max-w-md">
        ...
        {items.length === 0 ? (
          <div className="flex flex-1 flex-col items-center justify-center gap-4 px-6 text-center">
            ...
            <div>
              <p className="font-display text-xl text-foreground">{t('components.cartDrawer.empty')}</p>
              <p className="mt-1 text-sm text-muted-foreground">{t('components.cartDrawer.emptyMessage')}</p>
            </div>
            <Link
              to="/ara"
              onClick={closeCart}
            ...
            >
              {t('components.cartDrawer.startShopping')}
            </Link>
          </div>
        ) : (
          <>
                      ...
                      <div className="flex flex-1 flex-col">
                        <div className="flex items-start justify-between gap-2">
                          ...
                          <button
                            ...
                            disabled={busy}
                            aria-label={t('components.cartDrawer.remove')}
                        ...
                        <div className="mt-auto flex items-center justify-between pt-2">
                          <div className="flex items-center rounded-full border border-border">
                            <button
                              type="button"
                              aria-label={t('components.cartDrawer.decrease')}
                              disabled={busy}
                            ...
                            <button
                              type="button"
                              aria-label={t('components.cartDrawer.increase')}
            ...
            <div className="border-t border-border/60 px-6 py-5">
              <div className="flex items-center justify-between">
                <span className="text-sm text-muted-foreground">{t('components.cartDrawer.subtotal')}</span>
                <span className="font-display text-xl text-foreground">{formatPrice(subtotal)}</span>
              </div>
              <p className="mt-1 text-xs text-muted-foreground">{t('components.cartDrawer.shippingCalculated')}</p>
              <button
                type="button"
                onClick={goCheckout}
              ...
              >
                {t('components.cartDrawer.checkout')}
              </button>
              <div className="mt-3 flex items-center justify-between">
                ...
                >
                  {t('components.cartDrawer.viewCart')} <ArrowUpRight className="h-3.5 w-3.5" />
                </Link>
                <button
                  type="button"
                  onClick={closeCart}
                ...
                >
                  {t('components.cartDrawer.continueShopping')}
                </button>
              </div>
            </div>
          </>
